...
export default async function (
  ...
  try {
    ...
    const txStatus =
      txData.status === 200 &&
      txData.confirmed &&
      txData.confirmed.number_of_confirmations >= MIN_NUMBER_OF_CONFIRMATIONS
    ...
    if (txStatus === TransactionStatusE.CONFIRMED) {
      ...
      const block = txData.confirmed
        ? await arweave.blocks.get(txData.confirmed.block_indep_hash)
        : null;
